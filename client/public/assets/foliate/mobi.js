...
class PDB {
  ...
  async open(file) {
    ...
    this.#offsets = Array.from({ length: pdb.numRecords }, (_, i) => getUint(buffer.slice(i * 8, i * 8 + 4))).map((x, i, a) => [x, a[i + 1]])
  }
  loadRecord(index) {
    ...
    try {
      ...
      if (tocHref) {
        ...
        this.toc = Array.from(doc.querySelectorAll('a[filepos]')).reduce((arr, a) => {
          ...
          const level =
            indent > lastIndent ? lastLevel + 1 : indent === lastIndent ? lastLevel : (lastLevelOfIndent.get(indent) ?? Math.max(0, lastLevel - 1))
          if (level > lastLevel) {
            if (lastItem) {
              lastItem.subitems ??= []
...
const makePosURI = (fid = 0, off = 0) =>
  `kindle:pos:fid:${fid.toString(32).toUpperCase().padStart(4, '0')}:off:${off.toString(32).toUpperCase().padStart(10, '0')}`
...
class KF8 {
  ...
  async init() {
    ...
    try {
      ...
      const fdstTable = Array.from({ length: fdst.numEntries }, (_, i) => 12 + i * 8).map((offset) => [
