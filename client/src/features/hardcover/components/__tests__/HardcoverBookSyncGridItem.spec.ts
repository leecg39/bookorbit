...
import type { HardcoverBookSyncState, HardcoverSettings } from '@bookorbit/types'
import ToggleSwitch from '@/components/ui/ToggleSwitch.vue'
import { api } from '@/lib/api'
import { useHardcoverSettings } from '../../composables/useHardcoverSettings'
import HardcoverBookSyncGridItem from '../HardcoverBookSyncGridItem.vue'
...
const mocks = vi.hoisted(() => ({
  ...
  settings: {
    value: null as HardcoverSettings | null,
  },
  bookStates: new Map<number, HardcoverBookSyncState>(),
}))
...
function makeBookState(overrides: Partial<HardcoverBookSyncState> = {}): HardcoverBookSyncState {
  return {
    bookId: 12,
    syncOverride: null,
    syncEnabled: true,
...
function setBookState(bookId: number, overrides: Partial<HardcoverBookSyncState> = {}): HardcoverBookSyncState {
  const state = { ...makeBookState({ bookId }), ...overrides }
  mocks.bookStates.set(bookId, state)
  return state
}

function getBookState(bookId: number): HardcoverBookSyncState {
  return mocks.bookStates.get(bookId) ?? setBookState(bookId)
}

function mountItem(bookId = 12) {
  return mount(HardcoverBookSyncGridItem, { props: { bookId } })
}
...
describe('HardcoverBookSyncGridItem', () => {
  ...
  beforeEach(() => {
    ...
    mockApi.mockImplementation(async (input, init) => {
      ...
      if (url.endsWith('/hardcover/settings')) {
        return jsonResponse(mocks.settings.value)
      }

      const syncStateMatch = url.match(/\/hardcover\/books\/(\d+)\/sync-state$/)
      if (syncStateMatch) {
        ...
        if (init?.method === 'PATCH') {
          ...
          const nextState: HardcoverBookSyncState =
      ...
      const syncMatch = url.match(/\/hardcover\/books\/(\d+)\/sync$/)
      if (syncMatch) {
  ...
  it('shows the per-book sync state when Hardcover sync is enabled', async () => {
    const wrapper = mountItem()
    wrappers.push(wrapper)
    await flushPromises()
    await flushPromises()

    expect(mockApi).toHaveBeenCalledWith('/api/v1/hardcover/settings')
    expect(mockApi).toHaveBeenCalledWith('/api/v1/hardcover/books/12/sync-state')
    expect(wrapper.text()).toContain('Hardcover Sync')
  ...
  it('does not fetch per-book state when the user lacks Hardcover sync permission', async () => {
    ...
    expect(mockApi).not.toHaveBeenCalledWith('/api/v1/hardcover/settings')
    expect(mockApi).not.toHaveBeenCalledWith('/api/v1/hardcover/books/12/sync-state')
  })

  it('does not fetch per-book state when global Hardcover sync is disabled', async () => {
  ...
  it('shows a load error when the per-book state cannot be fetched', async () => {
    mockApi.mockImplementation(async (input) => {
      const url = String(input)
      if (url.endsWith('/hardcover/settings')) return jsonResponse(mocks.settings.value)
      if (url.endsWith('/hardcover/books/12/sync-state')) return jsonResponse({}, false)
      return jsonResponse({}, false)
    })
    ...
    expect(wrapper.text()).toContain('Unable to load Hardcover sync state.')
  })

  it('waits for fresh settings before using shared Hardcover settings state', async () => {
  ...
  it('rolls back the per-book exclusion setting when saving fails', async () => {
    mockApi.mockImplementation(async (input, init) => {
      const url = String(input)
      if (url.endsWith('/hardcover/settings')) return jsonResponse(mocks.settings.value)
      if (url.endsWith('/hardcover/books/12/sync-state') && init?.method === 'PATCH') return jsonResponse({ message: 'Forbidden' }, false)
      if (url.endsWith('/hardcover/books/12/sync-state')) return jsonResponse(getBookState(12))
      return jsonResponse({}, false)
    })
    ...
    expect(wrapper.findComponent(ToggleSwitch).props('modelValue')).toBe(true)
    expect(wrapper.text()).toContain('Failed to save Hardcover sync setting.')
  ...
  it('clears the saving state when a save finishes after the book changes', async () => {
    ...
    mockApi.mockImplementation(async (input, init) => {
      const url = String(input)
      if (url.endsWith('/hardcover/settings')) return jsonResponse(mocks.settings.value)
      if (url.endsWith('/hardcover/books/12/sync-state') && init?.method === 'PATCH') return patchResponse.promise
      if (url.endsWith('/hardcover/books/12/sync-state')) return jsonResponse(getBookState(12))
      if (url.endsWith('/hardcover/books/13/sync-state')) {
        return jsonResponse(
          makeBookState({
            bookId: 13,
            syncEnabled: true,
