...
const { retryFailed } = useBookMetadataFetchActions()

const page = ref(1)
const limit = 50
const data = ref<BookMetadataFetchFailedPage | null>(null)
...
async function loadPage(p: number) {
  loading.value = true
  try {
    const res = await api(`/api/v1/book-metadata-fetch/failed?page=${p}&limit=${limit}`)
    if (res.ok) {
