...
export interface BookDockFixtureRoot {
  rootPath: string;
  booksPath: string;
  bookDockPath: string;
...
export async function createBookDockFixtureRoot(prefix = 'book-dock-e2e-'): Promise<BookDockFixtureRoot> {
...
export function buildFb2Fixture(input: Fb2FixtureInput = {}): string {
  const title = input.title ?? 'Book Dock Fixture Title';
  const authors = input.authors ?? ['Book Dock Fixture Author'];
